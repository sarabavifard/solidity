...
/*
string to_string(RedundantStoreEliminator::Operation const& _op)
{
	return
		(_op.effect == RedundantStoreEliminator::Effect::Write ? "write to "s : "read from "s) +
		(_op.location == RedundantStoreEliminator::Location::Storage ? "storage "s : "memory "s) +
		(_op.start ? _op.start->str() : "?") + " + "s +
		(_op.length ? _op.length->str() : "?");
}
*/
...
void RedundantStoreEliminator::run(OptimiserStepContext& _context, Block& _ast)
{
	...
	RedundantStoreEliminator rse{_context.dialect, functionSideEffects, values};
	rse(_ast);
...
void RedundantStoreEliminator::operator()(ExpressionStatement const& _statement)
{
	...
	// TODO if length is zero, we can directly mark it as unused.

	if (!ranges::all_of(funCall->arguments, [](Expression const& _expr) -> bool {
		return get_if<Identifier>(&_expr) || get_if<Literal>(&_expr);
	}))
	{
		//cout << "Ignoring " << AsmPrinter{}(_statement) << endl;
		//cout << "not all arguments are variables." << endl;
		return;
	}
	...
	)
	{
		...
		//cout << "Adding store operation " << to_string(operations.front()) << endl;
...
void RedundantStoreEliminator::operator()(FunctionCall const& _functionCall)
{
	...
	//cout << "Handling function call " << AsmPrinter{}(_functionCall) << endl;
...
{
	//cout << "Applying operation " << to_string(_operation) << endl;
	for (auto& [statement, state]: m_stores)
	{
		//cout << " - store " << AsmPrinter{}(*statement) << endl;
		if (state == State::Undecided)
		{
			...
			{
				//cout << " - Changing to used: " << AsmPrinter{}(*statement) << endl;
				state = State::Used;
			}
			else if (_operation.effect == Effect::Write && knownCovered(storeOperation, _operation))
			{
				//cout << " - Changing to unused: " << AsmPrinter{}(*statement) << endl;
				state = State::Unused;
			}
		}
	}
